...
def webcam_feed(publishers, cap):
	global frame
...
def aggregate_faces(face_coord, args):
	global frame, past_faces
	...
	if len(face_coord) == past_faces.maxlen:
		past_faces.append(face_coord)

	best_face = map(lambda y: sum(y) / len(y), zip(*past_faces))
...
def main():
	...
	for i in xrange(len(publishers)):
		subscribers += [rospy.Subscriber('camera_faces_'+str(i), Int32Numpy, aggregate_faces, callback_args=[i, cap])]
	webcam_feed(publishers, cap)
