...
ros = [ [x[1],x[2].split(" ")] for x in val if x[0]=="ROS"]
erl = [ [x[1],x[2].split(" ")] for x in val if x[0]=="ERL"]
print ros
res = open("jaccard.idx","a+")

diff_1 = [jaccard_index([actual[int(x[1][0])-1], x[1][1]], "ros", x[0]) for x in enumerate(ros)]
diff_2 = [jaccard_index([actual[int(x[1][0])-1], x[1][1]], "erl", x[0]) for x in enumerate(erl)]
